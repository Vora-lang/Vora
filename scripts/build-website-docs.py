...
USER_GUIDE_DIFF = REPO_ROOT / "docs" / "15-写给用户.md"
WEBSITE_ROOT = REPO_ROOT.parent / "Vora-lang.github.io"
OUTPUT_FILE = WEBSITE_ROOT / "docs" / "language.html"
OUTPUT_FILE_USER_GUIDE = WEBSITE_ROOT / "docs" / "user-guide.html"
...
def build_user_guide_page(content_html: str) -> str:
    """Wrap user guide content HTML in the full website page template."""
    return f'''<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
<title>语言差异指南 — Vora 文档</title>
<link rel="icon" href="../vora_logo.svg" type="image/svg+xml" />
<style>
    *, *::before, *::after {{ margin: 0; padding: 0; box-sizing: border-box; }}
    :root {{
...
# ── Shared markdown → HTML processing ───────────────────────────────────────

def process_md_file(md_path: Path) -> str:
    """Read a markdown file, strip pre-content header, convert to HTML."""
...
def main():
    ...
    # Source → (output_path, build_template_fn, label)
    jobs = []

    if USER_GUIDE.exists():
        jobs.append((USER_GUIDE, OUTPUT_FILE, build_page, "language reference"))
    else:
        print(f"WARNING: {USER_GUIDE} not found, skipping language.html", file=sys.stderr)

    if USER_GUIDE_DIFF.exists():
        jobs.append((USER_GUIDE_DIFF, OUTPUT_FILE_USER_GUIDE, build_user_guide_page, "user guide"))
    else:
        print(f"WARNING: {USER_GUIDE_DIFF} not found, skipping user-guide.html", file=sys.stderr)
    ...
    for src_path, out_path, template_fn, label in jobs:
        content_html = process_md_file(src_path)
        page_html = template_fn(content_html)

        if check_only:
            if out_path.exists():
                existing = out_path.read_text(encoding="utf-8")
                if existing != page_html:
                    print(f"OUTDATED: {out_path.name} needs update — run: python scripts/build-website-docs.py")
                    all_ok = False
                else:
                    print(f"OK: {out_path.name} is up to date")
            else:
                print(f"MISSING: {out_path.name} does not exist — run: python scripts/build-website-docs.py")
                all_ok = False
        else:
