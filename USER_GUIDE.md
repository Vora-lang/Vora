...
| 位运算 `&` `\|` `^` `~` `<<` `>>` | **仍是 64 位**（见 §3 位运算符）：`1 << 64` → `0`；BigInt 参与位运算暂不支持，抛运行时错误 |
...
> 注意区分：**算术**是任意精度的，而**位运算仍是 64 位**。`1 << 62` 是精确的 `4611686018427387904`（不再是夹取值），但 `1 << 64` 依然按既有契约夹取为 `0`（右移负数为 `-1`）。BigInt 参与位运算暂不支持，会抛运行时错误。
