...
private:
    ...
    /// Depth of "top-level" expression contexts.
    ///
    /// Set to 1 around each call to `expression()` at statement-level
    /// entry points (`let`/`const` initializer, return value, bare
    /// expression statement). When > 0 AND we're not inside `(...)` or
    /// `[...]`, parsePrecedence's infix loop treats a newline as an
    /// implicit semicolon (Go-style ASI). See parsePrecedence for the
    /// firing rule.
    size_t asiDepth_ = 0;

    /// Current nesting depth of `(`/`[` pairs.
    ///
    /// Incremented by match() when LEFT_PAREN / LEFT_BRACKET is
    /// consumed; decremented on RIGHT_PAREN / RIGHT_BRACKET. ASI is
    /// suppressed when this is non-zero (newlines inside `(...)` and
    /// `[...]` are always allowed). `{` / `}` are not counted —
    /// blocks handle their own statement boundaries.
...
private:
    ...
    /**
     * @brief Derive the binding basename from a module import path.
     *
     * Strips directory components (forward slash or backslash) and any
     * trailing '.va' extension. Used when `import "path"` omits the
     * optional `as <name>` alias: the basename becomes the bound name.
     *
     * Examples:
     *   "math"         → "math"
     *   "std/json"     → "json"
     ...
     *   "C:/mod"       → "mod"
     *   "foo.va"       → "foo"
     *   "foo-bar"      → "foo-bar"   (kept verbatim — validity checked separately)
     *
     * @param modulePath Raw path string as it appeared between the quotes.
     * @return The basename; empty if @p modulePath ends in a separator.
     */
    static std::string deriveImportBaseName(const std::string& modulePath);

    /**
     * @brief Check whether @p name is a legal Vora identifier.
     *
     * A legal identifier starts with a letter or underscore and continues
     * with letters, digits, or underscores. Empty strings and names
     * containing punctuation (e.g. `foo-bar`, `foo.bar`) are rejected.
     *
     * @param name Candidate identifier name.
     * @return true iff @p name is a syntactically legal identifier.
     */
    static bool isLegalIdentifier(const std::string& name);
