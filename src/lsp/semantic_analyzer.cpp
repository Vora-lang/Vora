...
void SemanticAnalyzer::visitAwaitExpr(const AwaitExpr& expr) {
    // Await expression: the runtime suspends on `expr.value` and resumes
    // when it resolves. For symbol-collection purposes we walk the operand
    // so any identifiers it references are tracked normally.
    if (expr.value) visitExpr(*expr.value);
}
