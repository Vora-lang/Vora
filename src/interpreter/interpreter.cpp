...
// =========================================================================
// Array & String built-in method tables
// =========================================================================

static std::shared_ptr<NativeFunction> getArrayMethod(const std::string& name, std::shared_ptr<Array> arr) {
    if (name == "add") {
        ...
        return std::make_shared<NativeFunction>("remove", 1,
            [arr](const std::vector<Value>& args) -> Value {
                ...
                if (idx < 0 || static_cast<size_t>(idx) >= arr->elements.size()) {
                    throw RuntimeError("Index out of bounds for array.remove()",
                        Token(TokenType::IDENTIFIER, "remove", 0));
                }
        ...
        return std::make_shared<NativeFunction>("indexOf", 1,
            [arr](const std::vector<Value>& args) -> Value {
                for (size_t i = 0; i < arr->elements.size(); ++i) {
                    // Use value equality via the same index check
...
static std::shared_ptr<NativeFunction> getStringMethod(const std::string& name, std::string str) {
    if (name == "substring") {
...
Value Interpreter::visitPropertyExpr(const PropertyExpr& expr) {
    ...
    if (std::holds_alternative<std::shared_ptr<Array>>(obj)) {
        ...
        if (expr.property == "length")
            return static_cast<int64_t>(arr->elements.size());
        auto method = getArrayMethod(expr.property, arr);
        if (method) return method;
        throw RuntimeError("Unknown array method: " + expr.property, expr.dot);
    }
    ...
    if (std::holds_alternative<std::string>(obj)) {
        ...
        if (method) return method;
        throw RuntimeError("Unknown string method: " + expr.property, expr.dot);
    }
