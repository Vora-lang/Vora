...
> **时效性提示**：本文是对早期 Vora 的评价（其中「~10,000 行」等规模数字已明显过时，
> 正文提到类声明关键字处写作 `Obj`，现名 **`class`**）。正文保留原样不回改，
> 当前状态请以 `CHANGELOG.md` 与 `docs/00-roadmap.md` 为准。
...
评价：Vora 的 int64/float64 分离比 Lua 和 JS 的统一 number 设计更有利于性能优化（避免 boxed number），但不如 Python 的任意精度整数灵活。类型注解已实现并会进行运行时自动转换（如 `let x:float = 1` 得到 `1.0`）。
2.2 控制流
特性	Vora	Python 3	Lua 5.4	JavaScript
...
5. 类型注解的运行时转换（v0.25+ 已实现）：`let x:float = 1` 自动将整数转为浮点数。未来可考虑严格模式下的类型校验。
