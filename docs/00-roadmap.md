...
| 限制 | 严重度 | 说明 |
|------|--------|------|
| **闭包修改 + 局部递归 = VM 挂死** | 🔴 高 | 同时在一个局部函数内**修改捕获变量**且**递归调用自身**，VM 进入无限循环。单独使用（纯闭包修改 / 纯局部递归）均正常 |
| **局部函数自递归依赖编译器改动** | 🟡 中 | 函数名在编译函数体之前预分配为局部变量；这是 2026-06-15 刚修复的特性，边缘情况未充分测试 |
| **无尾调用优化 (TCO)** | 🟡 中 | 深度递归必然栈溢出；CPS 转换风格不现实 |
| **Upvalue 裸指针悬垂风险** | 🟡 中 | Upvalue 以原始 `Value*` 指向栈向量；若栈因 `push_back` 扩容重分配，指针全部作废——目前预分配 65536 槽勉强兜底 |

### 6.3 类型系统缺陷

| 缺陷 | 严重度 | 说明 |
|------|--------|------|
| **无 `const` / 不可变语义** | 🟡 中 | 变量永远可变；无法表达"此值不应被修改"的意图 |
| **无类型标注** | 🟡 中 | 纯动态类型；参数类型错误只在运行时暴露；无编译期检查 |
| **无泛型** | 🟢 低 | 无法参数化类型：`func identity(x) { return x }` 已是极限 |
| **`Value` 类型不可扩展** | 🟡 中 | `std::variant` 封闭了运行时类型集合；用户无法注册新类型（如 `Date`），只能通过 `Obj` 模拟 |
