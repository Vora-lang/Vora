...
Vora 最大的差异化价值是**作为 C++ 应用的嵌入式脚本**。但旧路线图对此几乎只字未提——C ABI 和 FFI 被排到 Q1 2027（一年后），而一些不紧急的语言特性却排在前面。
...
| 特性 | 为什么关键 | 工期 |
|------|-----------|------|
| **`std/fs` + `std/os` 标准库** | 无文件 I/O 无法做任何实际任务。这是从"玩具"到"工具"的质变。 | 1-2 周 |
| **`std/regex` 正则表达式** | 脚本语言的字符串处理核心能力。 | 1 周 |
| **空安全 `?.` + `??`** | 现代语言标配，消除 null 检查嵌套，代码量 -30%。JS 语法天然适配。 | 1 周 |
| **错误类型层级 + `catch (e if Type)`** | catch 无法按类型过滤 = 错误处理盲抓。Error 基类是 OOP 语言标配。 | 1 周 |

### 3.2 🟠 高优先级 — 提升语言体验

| 特性 | 为什么需要 | 工期 |
|------|-----------|------|
| **`do-while` 循环** | 基础控制流缺口。先执行后判断的场景（如输入验证）需要它。 | 1 天 |
| **调用端展开 `...expr`** | 与 rest 参数对称，解锁函数式模式。成本极低。 | 1-2 天 |
| **列表/Dict 推导式** | 一行替代多层循环，数据处理核心体验。JS/Python 风格。 | 1 周 |
| **命名参数** | `func(name="Vora", age=18)` 自文档化，减少参数顺序错误。 | 2-3 天 |
| **访问控制 `private`** | 所有成员公开 = 对象只是命名空间。封装是 OOP 的基础。 | 1 周 |
...
| **C ABI 导出** | 嵌入式脚本语言的核心能力。Lua 的成功很大程度上因为 C API。 | 1-2 周 |
...
| **C ABI FFI** | 调用外部 .dll/.so 中的 C 函数。 | 2 周 |
...
1. **嵌入优先**：C API 和 FFI 不再是"远期"，而是核心路线
2. **实用优先**：标准库 > 语法糖 > 性能优化
...
### Phase 3: 2027 Q1 (1-3月) — 嵌入能力 + 生产级 ⭐⭐

> 目标：让 Vora 能被 C++ 应用真正使用

```
嵌入能力
├── C ABI 导出（vora_c_api.h）
│   ├── vora_create_vm / vora_eval / vora_call / vora_destroy_vm
│   └── 任何语言都能宿主 Vora
└── C ABI FFI（调用外部 .dll/.so）
...
**为什么 C API 排在这里而非更晚**：Lua 的成功很大程度上因为 C API 极其简洁。Vora 要在嵌入式脚本市场与 Lua 竞争，C API 是基础设施。但需要先有完整的标准库（Phase 1），否则 C API 嵌入后也无法做有用的事情。
...
| 缺口 | 状态 | 说明 |
|------|------|------|
| **仅 C++ embedding API** | 🔴 待改进 | 无 C ABI 导出 |
| **无 FFI** | 🔴 待实现 | 无法调用 C 动态库 |
...
| **嵌入性差**（无 C ABI 导出） | Vora 天生为嵌入设计 | C ABI (Phase 3) |
...
| **C 扩展复杂**（CPython API 臃肿） | Vora C ABI 极简 | C ABI (Phase 3) |
