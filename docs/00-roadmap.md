...
**当前基线：`v0.30.0`（`v0.30-syntax-freeze`）—— 文法已冻结。**本文件记录的完成情况以 `CHANGELOG.md` 的 `[0.30.0]` 节为准。
...
| **任意精度整数**（`+` `-` `*` `%` 精确、比较精确、`/` 与 `**` 仍为 float） | ✅ | 2026-09 修复**静默夹取**：此前 ≥2^45 的整数（含字面量与运算结果）被静默夹到 `35184372088831`，20 位以上十进制字面量还会静默退化成 float。现按 §13 定案实现混合表示（内联 Int / 堆 GcBigInt），不变量「能内联的值绝不装箱」保证同一数值只有一个哈希/相等身份。位运算为**混合**：内联⊗内联保持 64 位（含文档化的移位计数夹取），任一操作数为 BigInt 时走 Python 式无限二补码任意精度（`2^45 << 40 == 2^85`、`~2^45 == -2^45-1`、`-6 & 3 == 2`）。见 `CHANGELOG.md` Breaking changes、`docs/19-bignum-value-design.md`、`tests/runtime/test_bigint.va`、`tests/unit/test_bigint.cpp` |
...
P2 项处置（v0.30 冻结前裁定，详见 `docs/18-v0.30-冻结计划.md`）：
  ...
  [X] 3.9 `as`：**仅改注释**，不补 cast 语法（补语法属新增能力，不在冻结范围）
  [X] 3.5 `..`/`..=`：**v1.0 不做**——属新增能力，后期补充不构成破坏性变更，故不占用冻结前的破坏性窗口
  接受现状（v1.0 不改）：3.1 `Obj` 体内 func 位置敏感、3.2 解构 cover grammar、
  3.3 `:` 四重重载、3.4 `for` 四重含义、3.7 `defer` 与 `finally` 并存

非 P2 的顺带修正（同批发现，均为「语法合法但语义非所见」）：
  [X] or-pattern 只认第一个备选 —— 编译器此前仅测试 `patterns[0]`，其余备选静默失效
...
> **formatter 与语言契约：绝不产出不可解析的输出、绝不静默改变语义；
> 纯表现层残留（位运算括号累积等）作为已知限制列明。**

在该契约之下，以下两项属于**纯表现层残留**，列为已知限制而非未完成项：

| 已知限制 | 影响 | 为何可接受 |
|---|---|---|
| 位运算操作数每跑一次 `vora fmt` 多套一层括号（`(6 & 3)` → `(6 & (3))`） | `vora fmt` 对含位运算的文件**不幂等** | 解析结果与运行行为完全一致，仅文本冗余；修复需重写括号插入的优先级判据（`docs/00-roadmap.md` P2 项） |
| 相对模块路径不被原样复现 | 格式化后行为可能改变（`examples/39_module_import_relative.va`、`tests/interpreter/test_module_export.va`） | 属 import 解析与格式化的交互，非静默语义变更；已在 `CHANGELOG.md` 的 Known limitations 中列明 |

除此之外，全语料库 162 个 `.va` 文件跑 `fmt -w` 后重跑，仅 6 个有差异，其中 4 个是打印
墙上时钟或自身 argv 路径（不可比），余下即上表两项。

**冻结后新增能力不构成破坏性变更**：`..` / `..=` 的 range/切片表达式（语法评审 3.5）
明确推迟到 v1.0 之后；语法评审 3.9 仅为注释修正；其余 P2 项（3.1 / 3.2 / 3.3 / 3.4 / 3.7）
接受现状并记入 EBNF 的「设计取舍」节。