...
> 作者按：本文从一线程序员的视角，基于对 Vora 源码（~8700 行 C++17）的完整阅读，剖析这门语言的设计哲学、实现质量、现有缺陷，并提出按优先级排序的用户体验完善路线图。2026-06-13 修订——反映 v0.17 最新状态。
...
> 6 周内从解释器原型演进到具备完整 VM 后端的跨平台语言——开发效率出色。

---

## 二、实现架构质量评估

### 2.1 整体印象：**A- 级代码**（~10500 行 C++17）

模块边界清晰，单一维护者下工程质量持续提升。零外部依赖，纯 C++17 + STL。双后端架构（解释器 + 字节码 VM）全部就绪，VM 为默认后端。跨平台构建系统（CMakePresets 20 个预设 + 交叉编译工具链）+ CI/CD 完备。

**代码量变化**：~6500 → ~9400 → ~10500 行（+62%），其中 VM 子系统从 ~1000 增至 ~3400 行（3.4×），贡献了主要增量。其余模块保持克制增长。
...
| 模块 | 行数 | 评价 |
|------|------|------|
| Lexer | 687 | 简洁高效：转义/Unicode/嵌套注释/错误恢复/0x/0o/0b 数字前缀就位。缺列号。 |
| Parser | 1185 | Pratt 实现优雅，panic-mode 恢复，14 Expr + 13 Stmt 种节点。无列号+错误消息简陋。 |
| AST | 1629 | 模板化 Visitor 模式，类型体系完整（27 种节点），泛型接口支持 3 种返回类型。 |
| Interpreter | 1621 | 功能充实：全运算符/短路求值/闭包/继承/异常。作为参考实现价值高。 |
| **Compiler+VM** | **~3400** | **项目核心**：50+ 条指令/栈式 VM/upvalue 闭包/异常全覆盖/全局驻留/快速数值操作码/常量折叠/属性拦截/Dict。v0.17 全特性完成，已可生产使用。 |
| Runtime | ~800 | `std::variant` 类型安全（10 类型），双数值系统+自动提升，环境链+闭包快照正确，`valueToString()`/`callDirectly()`/`markAsBoundMethod()`。 |
| 构建系统 | - | **A 级**：CMakePresets(20)+交叉编译(6)+CPack(4 格式)+CI(18 组合)+图标。同类项目中顶尖。 |

### 7.2 Vora 的差异化定位（v0.17 更新）

| 维度 | Lua 5.4 | Python 3 | JavaScript (V8) | Wren | Vora (v0.17) |
|------|---------|----------|----------------|------|--------------|
| 实现复杂度 | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| 语言简单性 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| 执行性能 | ⭐⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ (VM 原生操作码) |
| OOP 模型 | 原型式 | 类式 | 原型式 | 类式 | 类式(轻量+继承) |
| 标准库 | 极小 | 庞大 | 中等 | 小 | 极小（12 内建函数 + 18 内建方法） |
| 嵌入友好 | ⭐⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
...
### 7.3 与上一版（2026-05-31）的对比

| 指标 | 2026-05-31 | 2026-06-10 (v0.11) | 2026-06-13 (v0.17) |
|------|-----------|-------------------|-------------------|
| 总代码量 | ~6500 行 | ~9400 行 | ~8700 行 |
| VM 代码量 | ~1000 行（Phase 1） | 3175 行（Phase 1-4） | ~3400 行 |
| VM 指令数 | 32 | 50 | 50+（含 OP_CONSTANT_LONG / OP_DICT） |
| Value 类型数 | 7 | 7 | 11（+int64/FunctionPrototype/ClassData/Dict） |
| 数值类型 | double only | double only | int64 + float64（自动提升） |
| 内建函数 | 10 | 10 | 12（+len/type） |
| 内建方法 | 0 | 0 | 22（7 数组 + 11 字符串 + 4 字典） |
| VM 默认后端 | ❌（需 --vm 标志） | ✅ | ✅（解释器已移除） |
| VM 功能覆盖 | 表达式+全局变量+if/while/数组 | 全语言特性 | 全语言 + Dict + for-in 对象 + 默认参数 + C3 多继承 + super + 调用栈 + 错误增强 |
| 构建系统 | build.ps1 + Makefile | CMakePresets(20) + build.ps1 + build.sh | 不变 |
| 打包 | 无 | .msi/.deb/.rpm/.pkg.tar.xz | 不变 |
| 平台支持 | Windows | Windows + Linux + macOS（5 架构） | 不变 |
| CI | 无 | GitHub Actions（18 组合） | 不变 |
| 测试通过率 | 8/13 VM | 13/13 VM + 13/13 Interpreter | 20/20 VM |
| 异常处理 | 基本 try/catch | finally 路由 + 异常重抛 + handler 清理完备 | 不变 + 调用栈回溯 + 错误源码行 |

### 7.4 给贡献者的建议

1. ✅ **P0 已全部完成**（`len`、`type`、数组/字符串方法、int64/float64）——Vora 已从"能运行"跨越到"能写脚本"。
2. ✅ **P1 核心已全部完成**（默认参数、`super`、多继承 C3 MRO）——语言表达力大幅提升
3. **当前 P1**（`switch`/`match`、错误信息改进）——进一步提升开发体验
4. **模块系统 P2**（`import`/`export`）——赋予 Vora 多文件工程能力，这是从嵌入式脚本到独立语言的质变
5. **标准库起步**——哪怕只有 `math` + `io` 两个模块，也会极大改善开箱体验
6. **NaN Boxing**——如需性能，这是 ROI 最高的单项优化（但实现需要谨慎）
7. **LSP**——从语言到工具的最后一公里

---

> **后记（2026-06-13 修订）**：Vora 目前在 ~8700 行 C++ 代码内完成了一个完整的动态类型脚本语言实现。相比 6 月 10 日的 v0.12 版本，v0.13-v0.15 新增了默认参数、C3 多继承、`super` 关键字、运行时错误调用栈回溯、C++ 单元测试系统；v0.16 新增 Dict 字典类型、for-in 对象/字典迭代、VM 栈动态扩容（突破 1024 硬限制）、常量池 16 位索引；v0.17 新增错误信息全链路增强（源码行 + 插入符 ^ 位置标记）。P0 和 P1 核心需求已全部完成，Vora 的数据结构（Array + Dict + Object）和 OOP 特性（单继承 + 多继承 + C3 MRO + super）已达到生产级完整度。
>
> Vora 现在已经跨过了 P0 致命级门槛和 P1 核心需求：**Dict 字典 + for-in 全覆盖 + 默认参数 + 多继承 + `super` 让这门语言的数据处理和面向对象能力完整可用**。模块系统和标准库是下一步自然目标。

---

*分析日期：2026-06-13（修订）*

*基于 Vora 源码分支 `main`，~10500 行 C++17，v0.12*
