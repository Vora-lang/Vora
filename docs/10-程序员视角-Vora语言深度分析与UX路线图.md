...
> 作者按：本文从一线程序员的视角，基于对 Vora 源码（~9400 行 C++17）的完整阅读，剖析这门语言的设计哲学、实现质量、现有缺陷，并提出按优先级排序的用户体验完善路线图。2026-06-10 修订——反映 v0.11 最新状态。
...
### 2.7 Runtime（运行时系统）—— 评分：B+

**文件**：`src/runtime/` 目录，合计 719 行。

**亮点**：
- `Value` 使用 `std::variant`（7 种类型），类型安全。`std::visit` 用于模式匹配。
...
- `Value` 缺少类型查询接口——`std::holds_alternative<T>()` 散落在各处。
...
- 数组：`arr.insert(index, val)`、`arr.remove(index)`、`arr.indexOf(val)`
- 字符串：`str.upper()`、`str.lower()`、`str.trim()`、`str.split(delim)`、`str.replace(old, new)`
...
| 内建函数（10 个） | v0.5-v0.7 | print/clock/input/int/float/range/assert/bin/oct/hex |
...
| **Compiler+VM** | **3175** | **项目核心**：50 条指令/栈式 VM/upvalue 闭包/异常全覆盖/全局驻留/快速数值操作码/常量折叠。v0.11 已可生产使用。 |
| Runtime | 719 | `std::variant` 类型安全，环境链+闭包快照正确。缺类型查询接口。 |
| 构建系统 | - | **A 级**：CMakePresets(20)+交叉编译(6)+CPack(4 格式)+CI(18 组合)+图标。同类项目中顶尖。 |

### 7.2 Vora 的差异化定位（v0.11 更新）

| 维度 | Lua 5.4 | Python 3 | JavaScript (V8) | Wren | Vora (v0.11) |
|------|---------|----------|----------------|------|--------------|
| 实现复杂度 | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| 语言简单性 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| 执行性能 | ⭐⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ (VM 原生操作码) |
...
| 指标 | 2026-05-31 | 2026-06-10 | 变化 |
|------|-----------|-----------|------|
| 总代码量 | ~6500 行 | ~9400 行 | +45% |
| VM 代码量 | ~1000 行（Phase 1） | 3175 行（Phase 1-4） | 3× |
| VM 指令数 | 32 | 50 | +56% |
| VM 默认后端 | ❌（需 --vm 标志） | ✅ | — |
| VM 功能覆盖 | 表达式+全局变量+if/while/数组 | 全语言特性覆盖（闭包/对象/异常/插值） | — |
| 构建系统 | build.ps1 + Makefile | CMakePresets(20) + build.ps1 + build.sh | — |
| 打包 | 无 | .msi/.deb/.rpm/.pkg.tar.xz | — |
| 平台支持 | Windows | Windows + Linux + macOS（5 架构） | — |
| CI | 无 | GitHub Actions（18 组合） | — |
| 测试通过率 | 8/13 VM | 13/13 VM + 13/13 Interpreter | — |
| 异常处理 | 基本 try/catch | finally 路由 + 异常重抛 + handler 清理完备 | — |

### 7.4 给贡献者的建议

1. **立即解决 P0**（`len`、`type`、数组/字符串基本方法）——这是 Vora 从"能运行"到"能写脚本"的最后一块拼图。4 个任务总计 ~8 小时。
2. **然后补 P1**（默认参数、`super`、`switch`/`match`）——大幅提升语言表达力
3. **模块系统 P2**（`import`/`export`）——赋予 Vora 多文件工程能力，这是从嵌入式脚本到独立语言的质变
4. **标准库起步**——哪怕只有 `math` + `io` 两个模块，也会极大改善开箱体验
5. **NaN Boxing**——如需性能，这是 ROI 最高的单项优化（但实现需要谨慎）
6. **LSP**——从语言到工具的最后一公里

---

> **后记（2026-06-10 修订）**：Vora 目前在 ~9400 行 C++ 代码内完成了一个完整的动态类型脚本语言实现。相比 5 月 31 日的 ~6500 行版本，最大的变化是字节码 VM 从 Phase 1 原型进化到 Phase 1-4 完整后端（+ 性能优化 + 异常处理全覆盖），以及多架构构建系统和 CI/CD 基础设施的建立。这两个方向——语言内核的深化和工程基础设施的完善——同时推进，体现了项目维护者在语言实现和软件工程两个维度的均衡能力。
>
> Vora 现在处于一个微妙的临界点：**语言内核已经足够成熟，但标准库和生态完全空白**。P0 的 `len()`/数组方法/字符串方法是让这门语言"可以使用"的最低门槛。一旦跨过，Vora 将从一个"语言实现项目"转变为一个"可用的编程工具"。

---

*分析日期：2026-06-10（修订）*

*基于 Vora 源码分支 `main`，~9400 行 C++17，v0.11（commit `cfa6297`）*
