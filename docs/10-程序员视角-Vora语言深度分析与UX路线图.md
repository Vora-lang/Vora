...
> 作者按：本文从一线程序员的视角，基于对 Vora 源码（~10500 行 C++17）的完整阅读，剖析这门语言的设计哲学、实现质量、现有缺陷，并提出按优先级排序的用户体验完善路线图。2026-06-10 修订——反映 v0.12 最新状态。
...
| **Compiler+VM** | **~3400** | **项目核心**：50 条指令/栈式 VM/upvalue 闭包/异常全覆盖/全局驻留/快速数值操作码/常量折叠/属性拦截。v0.12 已可生产使用。 |
...
| 指标 | 2026-05-31 | 2026-06-10 (v0.11) | 2026-06-10 (v0.12) |
|------|-----------|-------------------|-------------------|
| 总代码量 | ~6500 行 | ~9400 行 | ~10500 行 |
...
| VM 默认后端 | ❌（需 --vm 标志） | ✅ | ✅ |
| VM 功能覆盖 | 表达式+全局变量+if/while/数组 | 全语言特性 | 全语言特性 + 双数值 + 方法 |
...
| 测试通过率 | 8/13 VM | 13/13 VM + 13/13 Interpreter | 13/13 VM + 13/13 Interpreter |
| 异常处理 | 基本 try/catch | finally 路由 + 异常重抛 + handler 清理完备 | 不变 |

### 7.4 给贡献者的建议

1. ✅ **P0 已全部完成**（`len`、`type`、数组/字符串方法、int64/float64）——Vora 已从"能运行"跨越到"能写脚本"。
2. **当前 P1**（默认参数、`super`、`switch`/`match`、错误信息改进）——大幅提升语言表达力和开发体验
...
> **后记（2026-06-10 修订）**：Vora 目前在 ~10500 行 C++ 代码内完成了一个完整的动态类型脚本语言实现。相比 5 月 31 日的 ~6500 行版本，最大的变化是字节码 VM 从 Phase 1 原型进化到 Phase 1-4 完整后端（+ 性能优化 + 异常处理全覆盖），多架构构建系统和 CI/CD 基础设施的建立，以及 v0.12 的 int64/float64 双数值类型、`len()`/`type()` 内建函数、17 个数组/字符串内建方法。这三个方向——语言内核的深化、工程基础设施的完善、程序员体验的提升——同时推进，体现了项目维护者在语言实现和软件工程两个维度的均衡能力。
>
> Vora 现在已经跨过了 P0 致命级门槛：**`len()`/数组方法/字符串方法/int64 类型让这门语言真正"可以使用"**。P1 的默认参数、`super`、`switch`/`match` 和模块系统是下一步自然目标。Vora 已从一个"语言实现项目"转变为一个"可用的编程工具"。
