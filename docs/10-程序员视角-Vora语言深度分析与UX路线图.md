...
> 作者按：本文从一线程序员的视角，基于对 Vora 源码（~8700 行 C++17）的完整阅读，剖析这门语言的设计哲学、实现质量、现有缺陷，并提出按优先级排序的用户体验完善路线图。2026-06-13 修订——反映 v0.15 最新状态。
...
| **Compiler+VM** | **~3400** | **项目核心**：50 条指令/栈式 VM/upvalue 闭包/异常全覆盖/全局驻留/快速数值操作码/常量折叠/属性拦截。v0.15 C3 多继承+super。已可生产使用。 |
...
### 7.2 Vora 的差异化定位（v0.15 更新）

| 维度 | Lua 5.4 | Python 3 | JavaScript (V8) | Wren | Vora (v0.15) |
|------|---------|----------|----------------|------|--------------|
| 实现复杂度 | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| 语言简单性 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| 执行性能 | ⭐⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ (VM 原生操作码) |
...
| 指标 | 2026-05-31 | 2026-06-10 (v0.11) | 2026-06-13 (v0.15) |
|------|-----------|-------------------|-------------------|
| 总代码量 | ~6500 行 | ~9400 行 | ~8700 行 |
| VM 代码量 | ~1000 行（Phase 1） | 3175 行（Phase 1-4） | ~3400 行 |
| VM 指令数 | 32 | 50 | 50 |
| Value 类型数 | 7 | 7 | 10（+int64/FunctionPrototype/ClassData） |
| 数值类型 | double only | double only | int64 + float64（自动提升） |
| 内建函数 | 10 | 10 | 12（+len/type） |
| 数组/字符串方法 | 0 | 0 | 18（7 数组 + 11 字符串） |
| VM 默认后端 | ❌（需 --vm 标志） | ✅ | ✅（解释器已移除） |
| VM 功能覆盖 | 表达式+全局变量+if/while/数组 | 全语言特性 | 全语言特性 + 双数值 + 方法 + 默认参数 + C3 多继承 + super + 调用栈 |
...
| 测试通过率 | 8/13 VM | 13/13 VM + 13/13 Interpreter | 16/16 VM |
| 异常处理 | 基本 try/catch | finally 路由 + 异常重抛 + handler 清理完备 | 不变 + 调用栈回溯 |
...
> **后记（2026-06-13 修订）**：Vora 目前在 ~8700 行 C++ 代码内完成了一个完整的动态类型脚本语言实现。相比 6 月 10 日的 v0.12 版本，v0.13-v0.15 新增了默认参数、C3 多继承、`super` 关键字、运行时错误调用栈回溯、C++ 单元测试系统（doctest）。P0 和 P1 核心需求已全部完成，Vora 的 OOP 特性（单继承 + 多继承 + super + C3 MRO）已达到生产级完整度。下一阶段的目标是模块系统和标准库——赋予 Vora 多文件工程能力。
>
> Vora 现在已经跨过了 P0 致命级门槛和 P1 核心需求：**默认参数、多继承、`super` 让这门语言的面向对象系统完整可用**。模块系统是下一步自然目标。Vora 已从"可用的编程工具"向"独立的脚本语言生态"迈进。
