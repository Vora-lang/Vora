...
- 块注释不支持嵌套（C 标准也不支持，但现代语言如 Rust/Swift 都支持了）。
...
2. **`+` 不能拼接字符串**
   ```vora
   let msg = "Hello " + "World"  // ❌ RuntimeError
   ```
   程序员的本能反应被打破。字符串拼接是最基本的日常需求。
...
5. **解析错误立即终止**——只报告第一个错误，修正后可能立即遇到第二个，迭代修复极慢。

6. **错误信息不含列号**——只有行号，在长行中定位困难。

7. **没有 `break` / `continue`**——循环控制的基本构建块缺失。

8. **`for-in` 只支持数组**——`for ch in "hello"` 无法工作。

9. **没有类型查询函数**——你无法在运行时知道一个值是什么类型。

10. **`shared_ptr` 循环引用导致内存泄漏**——对象属性可以引用自身，引用计数永不为零。
...
17. **没有测试框架**——无法自动化验证语言行为。

18. **没有模块系统**——所有代码必须写在一个文件里。

19. **REPL 不能多行输入**——无法在 REPL 中定义函数。

20. **没有标准库**——文件 IO、字符串操作、数学函数等都需要手写。

21. **不支持 Unicode 标识符**——`isAlpha()` 只检查 ASCII。
...
**实现要点**（`src/interpreter/interpreter.cpp:479-547`）：
在 `visitBinaryExpr` 的类型检查中，在数字分支后面增加：
```cpp
